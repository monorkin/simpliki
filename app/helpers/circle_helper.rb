...
module CircleHelper
  ...
  def exercise_circle(exercise, **options)
    content_tag(
      :div,
      class: "relative flex text-white size-64",
      data: { controller: "exercise", exercise_steps_value: serialize_exercise(@exercise) }
    ) do
      content_tag(:div, "", class: "absolute inset-0 z-40 flex justify-center cursor-pointer", data: { action: "click->exercise#toggle", exercise_target: "overlay" }) +
      animated_circle(symbol: dom_id(exercise, :breath_guide), class: "hidden") +
      animated_circle(use: dom_id(exercise, :breath_guide), class: "absolute inset-0 z-30 w-full transition-all duration-300", stroke: "black", stroke_width: 2, name: "exercise-circle-min", data: { exercise_target: "minCircle" }) +
      animated_circle(use: dom_id(exercise, :breath_guide), class: "absolute inset-0 z-20 w-full transition-all", data: { exercise_target: "progressCircle" }) +
      animated_circle(use: dom_id(exercise, :breath_guide), class: "absolute inset-0 z-10 w-full opacity-10", name: "exercise-circle-max", data: { exercise_target: "maxCircle" })
    end
  end
